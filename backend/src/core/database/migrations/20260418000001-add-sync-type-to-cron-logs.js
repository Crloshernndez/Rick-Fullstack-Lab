...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.addColumn('cron_logs', 'sync_type', {
      type: Sequelize.ENUM('initial', 'scheduled', 'manual'),
      allowNull: false,
      defaultValue: 'scheduled',
      comment: 'Type of synchronization: initial (first run), scheduled (cron job), or manual (user-triggered)',
    });
  ...
  async down(queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.query('DROP TYPE IF EXISTS "enum_cron_logs_sync_type";');
  }
};
